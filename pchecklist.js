...
{
    var stringPassword, arrayBadPasswords, stringOutput, intCount; // declares variables
    
    stringPassword = $("password").value; // receives user input
    ...
    stringOutput = "This is a good password."; // sets default output
    
  if (stringPassword.length < 8)
   {
    stringOutput = "Sorry,  your password must be more than 8 characters!"; 
   }
   else
   {
     for (intCount = 0; intCount < arrayBadPasswords.length; intCount++) // this loop checks user's input against the array of bad passwords
     {
        if (stringPassword === arrayBadPasswords[intCount])
        {
            stringOutput = "This is a bad password. Try again!"; // alerts user that they entered a bad password
        }
     }
   }
   $("output").value = stringOutput; // pushes output back to the DOM
};
